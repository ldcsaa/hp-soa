...
public class ContextConfig implements ServletContextListener
{
    ...
    @Value("${" + LoggingApplicationListener.REGISTER_SHUTDOWN_HOOK_PROPERTY + ":true}")
    private boolean registerLoggingShutdownHook;

    public ContextConfig(@Qualifier(LoggingApplicationListener.LOGGING_SYSTEM_BEAN_NAME) ObjectProvider<LoggingSystem> loggingSystem)
    {
        this.loggingSystem = loggingSystem.getIfUnique();
    }
    ...
    private static void checkProxy(ProxyProperties proxy)
    {
        if(!proxy.isEnabled())
            return;
    ...
    {
        if(registerLoggingShutdownHook || loggingSystem == null || loggingSystem.getShutdownHandler() == null)
            return;
